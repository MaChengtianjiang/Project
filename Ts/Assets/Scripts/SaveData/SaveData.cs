...
public class SaveData {
	...
	private static SaveDataBase Savedatabase {
		get {
			if (savedatabase == null) {
				...
				string fileName = Application.companyName + ".ywapp2.s";
	...
	//------------------------------------------------------------
	/// <summary>
	/// 指定したキーとT型のクラスコレクションをセーブデータに削除します。
	/// </summary>
	/// <typeparam name="T">ジェネリッククラス</typeparam>
	/// <param name="key">キー</param>
	/// <param name="list">T型のList</param>
	/// <exception cref="System.ArgumentException"></exception>
	/// <remarks>指定したキーとT型のクラスコレクションをセーブデータに削除します。</remarks>
	//------------------------------------------------------------
	public static void RemoveList<T>(string key, List<T> list) {
		Savedatabase.RemoveList<T>(key, list);
	}
	...
	//------------------------------------------------------------
	/// <summary>
	///  指定したキーとT型のクラスコレクションをセーブデータから削除します。
	/// </summary>
	/// <typeparam name="T"></typeparam>
	/// <param name="key"></param>
	/// <param name="obj"></param>
	/// <exception cref="System.ArgumentException"></exception>
	//------------------------------------------------------------
	public static void RemoveClass<T>(string key, T obj) where T : class, new() {
		Savedatabase.RemoveClass<T>(key, obj);
	}
	...
	//------------------------------------------------------------
	/// <summary>
	/// セーブクラウドからダウンロードしたデータで上書きします。
	/// </summary>
	//------------------------------------------------------------
	public static void Downloading(string json) {
		Savedatabase.Downloading(json);
	}

	//------------------------------------------------------------
	/// <summary>
	/// セーブクラウドにデータをアップロードします。
	/// </summary>
	//------------------------------------------------------------
	public static string Uploading() {
		return Savedatabase.Uploading();
	}
	...
	/// </summary>
	//------------------------------------------------------------
	public static T GetClassFromJson<T>(string json, string key, T _default) where T : class, new() {
		return Savedatabase.GetClassFromJson<T>(json, key, _default);
	}
	...
	private class SaveDataBase {
		...
		public void SetList<T>(string key, List<T> list) {
			...
			string json = UnityEngine.JsonUtility.ToJson(serializableList);
		...
		public List<T> GetList<T>(string key, List<T> _default) {
			...
			Serialization<T> deserializeList = UnityEngine.JsonUtility.FromJson<Serialization<T>>(json);

			return deserializeList.ToList();
		}

		public void RemoveList<T>(string key, List<T> list) {
			keyCheck(key);
			if (saveDictionary.ContainsKey(key)) {
		...
		public void RemoveClass<T>(string key, T obj) where T : class, new() {
			keyCheck(key);
			if (saveDictionary.ContainsKey(key)) {
				saveDictionary.Remove(key);
			}
		...
		public void Save() {
			using (StreamWriter writer = new StreamWriter(path + fileName, false, Encoding.GetEncoding("utf-8"))) {
				...
				string dictJsonString = UnityEngine.JsonUtility.ToJson(serialDict);
				Crypt.EncryptPassphraseFunc = FuncKey;
				Crypt.EncryptSaltFunc = FuncSalt;
				dictJsonString = Crypt.Encrypt(dictJsonString);
				Crypt.EncryptPassphraseFunc = FuncEmpty;
				Crypt.EncryptSaltFunc = FuncBytesEmpty;
			...
			if (File.Exists(path + fileName)) {
				using (StreamReader sr = new StreamReader(path + fileName, Encoding.GetEncoding("utf-8"))) {
					if (saveDictionary != null) {
						string dictJsonString = sr.ReadToEnd();
						Crypt.EncryptPassphraseFunc = FuncKey;
						Crypt.EncryptSaltFunc = FuncSalt;
						dictJsonString = Crypt.Decrypt(dictJsonString);
						Crypt.EncryptPassphraseFunc = FuncEmpty;
						Crypt.EncryptSaltFunc = FuncBytesEmpty;
						var sDict = UnityEngine.JsonUtility.FromJson<Serialization<string, string>>(dictJsonString);
		...
		public void Downloading(string json) {
			using (StreamWriter writer = new StreamWriter(path + fileName, false, Encoding.GetEncoding("utf-8"))) {
				writer.WriteLine(json);
			}
			if (!string.IsNullOrEmpty(json)) {
				string dictJsonString = json;
				Crypt.EncryptPassphraseFunc = FuncKey;
				Crypt.EncryptSaltFunc = FuncSalt;
				dictJsonString = Crypt.Decrypt(dictJsonString);
				Crypt.EncryptPassphraseFunc = FuncEmpty;
		...
		public string Uploading() {
			Save();
			string dictJsonString = "";
			if (File.Exists(path + fileName)) {
				using (StreamReader sr = new StreamReader(path + fileName, Encoding.GetEncoding("utf-8"))) {
					if (saveDictionary != null) {
						dictJsonString = sr.ReadToEnd();
					}
				}
			}
			return dictJsonString;
		}
		...
		/// </summary>
		//------------------------------------------------------------
		public T GetClassFromJson<T>(string json, string key, T _default) where T : class, new() {
			if (string.IsNullOrEmpty(json)) {
				return _default;
			}
			...
			var sDict = UnityEngine.JsonUtility.FromJson<Serialization<string, string>>(dictJsonString);
			sDict.OnAfterDeserialize();
			var dictionary = sDict.ToDictionary();
			...
			string tempJson = dictionary[key];
			T obj = UnityEngine.JsonUtility.FromJson<T>(tempJson);
			return obj;
		}
		...
		private const string CryptKey = @"p9seOjHa8vh6gkRn";
		private const string CryptSalt = @"nAaDsj4aN5gaTnka";
		...
		/// </summary>
		//------------------------------------------------------------
		private static string FuncEmpty() {
			return "";
		}
		...
		/// </summary>
		//------------------------------------------------------------
		private static byte[] FuncSalt() {
			return System.Text.Encoding.UTF8.GetBytes(CryptSalt);
		}
		...
		/// </summary>
		//------------------------------------------------------------
		private static byte[] FuncBytesEmpty() {
